...
const initialState: Settings = {
  ...
  themes: [
    { id: '1', name: 'Dark', selected: false, applied: false },
    { id: '2', name: 'Light', selected: false, applied: true },
...
export const applyMouseData = createAsyncThunk(
  ...
  async (data:any, {dispatch,getState}) => {
      ...
      let viewerId = rootState.app.viewers[rootState.app.activeViewer || ""];
       setMouseBindings(json, viewerId);
  }
)
...
export const settingsSlice = createSlice({
  ...
  reducers: {
    ...
    // Paste selected item controls

      state.menuItems.map((item)=> {

        if(item.id === action.payload.activeUserID) {

            if(item.type === Source.SYSTEM) {

               state.systemProvided.map((item)=>{
      ...
      if(undoable) {
        ...
        undoStack.add({
          ...
          redo:{reducer:pasteItem,payload:{activeUserID:action.payload.activeUserID,id:action.payload.id}}
        })
      }
