...
import {descend, prop, sortWith,ascend} from "ramda";
import { setUndo } from './displayModesSlice';
...
export const addMessageAsync = createAsyncThunk(
    "message/addMessageAsync",
   async (data:addMessageData,{dispatch,getState}) => {
    let root = getState() as RootState;
    let undoRedo = root.displayModes.undoable;
    if(undoRedo){
...
export const messageSlice = createSlice({
    ...
    reducers: {
        ...
        updateMessage: (state, action: PayloadAction<{id:string, transferredSize:number | string}>) => {
        ...
        editSearch :(state,action : PayloadAction<any>) => {
...
export const sortedNotification = (state : RootState):NotificationList[] => {
    ...
    const sortedNotificationList = sortWith([ascend(prop("time"))], state.message.notificationLists);
