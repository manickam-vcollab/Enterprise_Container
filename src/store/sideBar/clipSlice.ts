...
import {
  getNormalizedEqn,
  getPerpendicular,
  getWorldTransformFromPlaneEqn,
  planeEqnFrom3pts,
} from "../../components/utils/Math";
...
const getRootPlane = (state: planes, id: number): number => {
  ...
  if (
    (curPlane.masterPlane.id !== null ||
      curPlane.masterPlane.id !== undefined) &&
    curPlane.masterPlane.id > -1
  ) {
    let parentIdx = state.planes.findIndex(
      (item) => item.id === curPlane.masterPlane.id
    );
...
export const fetchSectionPlaneData = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    let result: any = [];
    if (viewerId) result = getSectionGUIData(viewerId);
...
export const setSectionPlaneData = createAsyncThunk(
  ...
  async (data: { id: number }, { dispatch, getState }) => {
    const rootState = getState() as RootState;
    const viewerId = rootState.app.viewers[rootState.app.activeViewer || ""];
    const index: any = rootState.clipPlane.planes.findIndex(
    ...
    setSectionPlaneEquation(
      data.id,
      new Float32Array(curPlane.worldTransform),
      new Float32Array(curPlane.initTransform),
      viewerId
    );
...
const generateEqn = (
  ...
  if (planes.length === 0) {
    ...
    return getNormalizedEqn([n[0], n[1], n[2], vec3.dot(n, c)]);
  } else if (planes.length === 1) {
    const plane1 = planes[0];
    let n1 = vec3.fromValues(
      plane1.worldTransform[8],
      plane1.worldTransform[9],
      plane1.worldTransform[10]
    );
    let n2 = getPerpendicular(n1);
    return getNormalizedEqn([n2[0], n2[1], n2[2], vec3.dot(n2, c)]);
  } else {
    const plane1 = planes[0];
    ...
    let n2 = vec3.fromValues(
      plane2.worldTransform[8],
      plane2.worldTransform[9],
      plane2.worldTransform[10]
    );
    let n3 = vec3.create();
    vec3.cross(n3, n1, n2);
    return getNormalizedEqn([n3[0], n3[1], n3[2], vec3.dot(n3, c)]);
  }
};
...
export const addPlane = createAsyncThunk(
  ...
  async (data: { undoable: boolean }, { dispatch, getState }) => {
    ...
    let bbox = getSceneBoundingBox(false, viewerId);
    ...
    if (data.undoable) {
      undoStack.add({
        undo: {
          reducer: removePlane,
          payload: { id: id, redoIncrement: true },
        },
...
export const duplicatePlane = createAsyncThunk(
  ...
  async (
    data: { pastedPlane: plane; undoable?: boolean },
    { dispatch, getState }
  ) => {
    ...
    if (data.undoable) {
      undoStack.add({
        undo: {
          reducer: removePlane,
          payload: { id: cloneId, redoIncrement: true },
        },
        redo: {
          reducer: duplicatePlane,
          payload: { pastedPlane: data.pastedPlane },
        },
      });
    }
  }
...
export const removePlane = createAsyncThunk(
  "clipSlice/removePlane",
  async (
    data: { id: number; redoIncrement?: boolean; undoable?: boolean },
    { dispatch, getState }
  ) => {
    const rootState = getState() as RootState;

    const deletedPlane = rootState.clipPlane.planes.find(
      (item) => item.id === data.id
    );
    const indexOfDeletedPlane = rootState.clipPlane.planes.findIndex(
      (item) => item.id === data.id
    );
    ...
    if (data.redoIncrement === true) dispatch(clipSlice.actions.decrementId());

    if (data.undoable === true) {
      undoStack.add({
        undo: {
          reducer: undoDelete,
          payload: { plane: deletedPlane, deletedIndex: indexOfDeletedPlane },
        },
...
const undoDelete = createAsyncThunk(
  "clipSlice/undoDelete",
  async (
    data: { plane: plane; deletedIndex: number },
    { dispatch, getState }
  ) => {
    const rootState = getState() as RootState;
    ...
    addSectionPlane(
      plane.id,
      new Float32Array(plane.worldTransform),
      plane.color,
      viewerId
    );
...
export const setActive = createAsyncThunk(
  ...
  async (data: { clicked: plane }, { dispatch, getState }) => {
    ...
    const rootState = getState() as RootState;
    const selectedPlane = rootState.clipPlane.planes.filter(
    ...
    else setActiveSectionPlane(-1, viewerId);
  }
);
...
export const setSelectionMode = createAsyncThunk(
  ...
  async (
    data: { activeId: number; selectionMode: SelectionMode },
    { dispatch, getState }
  ) => {
...
export const clipSlice = createSlice({
  ...
  reducers: {
    ...
    pushPlane: (
      state,
      action: PayloadAction<{ plane: plane; index: number }>
    ) => {
    ...
    pastePlane: (state, action: PayloadAction<plane>) => {
      if (state.planes.length < state.settings.maxAllowedPlanes) {
        ...
        const masterPlaneIndex = state.planes.findIndex(
          (item) => item.id === clone.masterPlane.id
        );
    ...
    deletePlane: (state, action: PayloadAction<number>) => {
      ...
      if (index >= 0) {
        ...
        if (masterPlaneId > -1) {
          const masterIndex = state.planes.findIndex(
            (item) => item.id === masterPlaneId
          );
          let changeItem = state.planes[masterIndex];
          changeItem.childPlane = changeItem.childPlane.filter(
      ...
      const newArray = state.planes.filter(
        (item) => item.id !== action.payload
      );
      state.planes = [...newArray];
    },

    reInitializeALLPlane : (state, action: PayloadAction<void>) =>{    
      state.planes = [];
      state.settings.selectionMode = SelectionMode.NONE;
      state.settings.idGenerator = -1;
      state.settings.maxAllowedPlanes = 6;
      state.settings.planesData = [];
    },

    editEquationGUI: (
      state,
      action: PayloadAction<{ id: number; eqn: EqnGUI }>
    ) => {
    ...
    editNormalInverted: (state, action: PayloadAction<number>) => {
      const index: any = state.planes.findIndex(
        (item) => item.id === action.payload
      );
      if (index >= 0) {
        let changeItem: plane = state.planes[index];
        changeItem.clipNormalInverted = !changeItem.clipNormalInverted;
        clipSlice.caseReducers.invert(state, {
          payload: { id: action.payload },
          type: "clipSlice/invert",
        });
        state.planes[index] = changeItem;
      }
    },
    ...
    editTranslate: (
      state,
      action: PayloadAction<{ id: number; translate: number }>
    ) => {
      const index: any = state.planes.findIndex(
        (item) => item.id === action.payload.id
      );
      if (index >= 0) {
        ...
        let data = {
          ...
          delta: action.payload.translate - changeItem.translate,
        };
        changeItem.translate = action.payload.translate;
        clipSlice.caseReducers.translate(state, {
          payload: data,
          type: "clipslice/translate",
        });
        // state.planes[index] = changeItem;
      }
    },
    ...
    editRotate: (
      state,
      action: PayloadAction<{ id: number; rotate: number }>
    ) => {
      const index: any = state.planes.findIndex(
        (item) => item.id === action.payload.id
      );
      if (index >= 0) {
        ...
        let data = {
          id: action.payload.id,
          delta: action.payload.rotate - changeItem.rotate,
          axis: Axis.Z,
        };
        clipSlice.caseReducers.rotate(state, {
          payload: data,
          type: "clipslice/rotate",
        });
    ...
    editAxisX: (
      state,
      action: PayloadAction<{ id: number; axisX: number }>
    ) => {
      const index: any = state.planes.findIndex(
        (item) => item.id === action.payload.id
      );
      if (index >= 0) {
        ...
        let data = {
          id: action.payload.id,
          delta: action.payload.axisX - changeItem.axisX,
          axis: Axis.X,
        };
        clipSlice.caseReducers.rotate(state, {
          payload: data,
    ...
    editAxisY: (
      state,
      action: PayloadAction<{ id: number; axisY: number }>
    ) => {
      const index: any = state.planes.findIndex(
        (item) => item.id === action.payload.id
      );
      if (index >= 0) {
        ...
        let data = {
          id: action.payload.id,
          delta: action.payload.axisY - changeItem.axisY,
          axis: Axis.Y,
        };
        clipSlice.caseReducers.rotate(state, {
          payload: data,
    ...
    editPlaneName: (
      state,
      action: PayloadAction<{ id: number; editName: string; undoable: boolean }>
    ) => {
      const index: any = state.planes.findIndex(
        (item) => item.id === action.payload.id
      );
      ...
      if (action.payload.undoable) {
        undoStack.add({
          undo: {
            reducer: editPlaneName,
            payload: { id: action.payload.id, editName: oldName },
          },
          redo: {
            reducer: editPlaneName,
            payload: {
              id: action.payload.id,
              editName: action.payload.editName,
            },
          },
        });
      }
    ...
    saveSelectedPlane: (state, action: PayloadAction<{ clicked: plane }>) => {
      const alreadySeletedIndex = state.planes.findIndex(
        (item) => item.selected === true
      );
      const index = state.planes.findIndex(
        (item) => item.id === action.payload.clicked.id
      );
    ...
    undoMinMaxGUI: (
      state,
      action: PayloadAction<{ id: number; min: number; max: number }>
    ) => {
      const index: any = state.planes.findIndex(
        (item) => item.id === action.payload.id
      );
    ...
    updateEqnGUI: (state, action: PayloadAction<{ id: number | string }>) => {
      ...
      let curPlane = state.planes[index];
      let inputNormal = vec3.fromValues(
        curPlane.userInputEquation[0],
        curPlane.userInputEquation[1],
        curPlane.userInputEquation[2]
      );
      ...
      if (curPlane.childPlane.length > 0) {
        curPlane.childPlane.forEach((planeId) => {
          clipSlice.caseReducers.updateEqnGUI(state, {
            payload: { id: planeId },
            type: "clipSlice/updateEqn",
          });
        });
      }
    },
    updateEqn: (
      state,
      action: PayloadAction<{ id: number | string; viewerId: any }>
    ) => {
      ...
      if (curPlane.masterPlane.id > -1) {
        let parentIndex = state.planes.findIndex(
          (item) => item.id === curPlane.masterPlane.id
        );
      ...
      clipSlice.caseReducers.update(state, {
        payload: { id: id as number },
        type: "clipSlice/update",
      });
    },
    update: (state, action: PayloadAction<{ id: number }>) => {
      ...
      clipSlice.caseReducers.updatePlaneMatrix(state, {
        payload: { id: rootPlaneId, parentMat: null },
    ...
    updatePlaneMatrix: (
      state,
      action: PayloadAction<{ id: number; parentMat: mat4 | null }>
    ) => {
    ...
    rotate: (
      state,
      action: PayloadAction<{ id: number; delta: number; axis: Axis }>
    ) => {
    ...
    setChildPlane: (
      state,
      action: PayloadAction<{ childId: number; masterId: number }>
    ) => {
      const childIndex = state.planes.findIndex(
        (item) => item.id === action.payload.childId
      );
      if (childIndex >= 0) {
        if (state.planes[childIndex].masterPlane.id === -1) {
          const index = state.planes.findIndex(
            (item) => item.id === action.payload.masterId
          );
          if (index >= 0) {
            let changeItem: any = state.planes[index];
            if (
              changeItem.childPlane.includes(action.payload.childId) === false
            ) {
              changeItem.childPlane = [
                ...changeItem.childPlane,
        ...
        if (state.planes[childIndex].masterPlane.id > -1) {
          const currentMasterIndex = state.planes.findIndex(
            (item) => item.id === state.planes[childIndex].masterPlane.id
          );
          if (currentMasterIndex >= 0) {
            let changeMaster: any = state.planes[currentMasterIndex];
            changeMaster.childPlane = changeMaster.childPlane.filter(
              (item: number) => item !== action.payload.childId
            );
            state.planes[currentMasterIndex] = changeMaster;
          }
          const newMasterIndex = state.planes.findIndex(
            (item) => item.id === action.payload.masterId
          );
          if (newMasterIndex >= 0) {
    ...
    setMasterPlane: (
      ...
      if (index >= 0) {
        let changeItem: plane = state.planes[index];
        changeItem.masterPlane = {
          id: masterId,
          name: action.payload.masterName,
        };
    ...
    updateState: (state, action: PayloadAction<Partial<planes>>) => {
      const { payload } = action;
      if (payload.planes) state.planes = payload.planes;
...
export const selectedPlane = (state: RootState) => {
  const clickedValues = state.clipPlane.planes.filter(
    (item) => item.selected === true
  );
...
export const selectActivePlane = (state: RootState) => {
  ...
  if (clickedValues.length === 1) return clickedValues[0].id;
  else return -1;
};

export const updateStateAsync = createAsyncThunk(
  "clipSlice/updateStateAsync",
  async (data: Partial<planes>, { dispatch, getState }) => {
    const rootState = getState() as RootState;
    const viewerId = rootState.app.viewers[rootState.app.activeViewer || ""];
    
    //Clean up existing clipdata in store
    if (rootState.clipPlane.planes?.length) {
      const planeIds = rootState.clipPlane.planes.map((item) => item.id);
      planeIds.forEach((eachID) => {
        //dispatch(editEnabled({ id: eachID, isEnabled: false }));
        //dispatch(setSectionPlaneData({ id: eachID }));
        deleteSectionPlane(eachID, viewerId);
        //dispatch(clipSlice.actions.deletePlane(eachID));
      });
      await dispatch(clipSlice.actions.reInitializeALLPlane());
    } 
    await dispatch(clipSlice.actions.updateState(data));    
    if (data?.planes?.length) {
      data?.planes?.map(async (item) => {
        addSectionPlane(item.id,new Float32Array(item.worldTransform),item.color, viewerId);
        //dispatch(editEnabled({ id: item.id, isEnabled: item.enabled }));
        await dispatch(setSectionPlaneData({ id: item.id }));
      });
    }    
  }
);
