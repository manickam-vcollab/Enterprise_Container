export default function simulatedAnnealingLabeler (label_array, anchor_array, width, height, numberOfSweeps) {
	var lab = label_array,
		anc = anchor_array,
		w = width, // box width
		h = height, // box width
	...
	// energy function, tailored for label placement
		...
		for (var i = 0; i < m; i++) {
          ...
          // penalty for label-boundary overlap
          if (x22  >  w - viewportMargin) {
            x_overlap  =  x21 > w? lab[index].x : x22  -  w;
            y_overlap  =  lab[index].height;
            overlap_area  =  x_overlap  *  y_overlap;
            ener  += (overlap_area  *  w_lab_anc);
          }
          if (y21 < viewportMargin) {
            x_overlap = lab[index].width;
            y_overlap = y21 < 0 ? lab[index].height : lab[index].height - y21;
            overlap_area  =  x_overlap  *  y_overlap;
            ener  += (overlap_area  *  w_lab_anc);
          }
          if (x21 < viewportMargin) {
            x_overlap = x22 < 0 ? lab[index].width : -x21;
            y_overlap = lab[index].height;
            overlap_area  =  x_overlap  *  y_overlap;
            ener  += (overlap_area  *  w_lab_anc);
          }
          if (y21 > h - viewportMargin) {
            x_overlap = lab[index].width;
            y_overlap = y22 > h ? lab[index].height : y21 - h;
            overlap_area  =  x_overlap  *  y_overlap;
            ener  += (overlap_area  *  w_lab_anc);
          }
