...
import { selectActionItemLength, tourListSlice } from 'store/tourSlice'
...
function Body(props: BodyProps) {
    ...
    const actionItemLength=useAppSelector(selectActionItemLength)
...
const tourFunc=(event)=>{
  if(actionItemLength >= 2 ){
  if(event.target.checked ){
    
    dispatch(tourListSlice.actions.setUpdateAction(tutorialSteps.steps[5].target.slice(1)))
  } else
  { dispatch(tourListSlice.actions.setUpdateDecreAction(tutorialSteps.steps[5].target.slice(1))) }
}
}
  ...
  return (
    <div ref = {containerRef} style={{height:'100%', overflow:'hidden'}} >
         ...
         return(<ListItem id={e.id} button onClick={() => handleResultsClick(e)}>
    ...
    >
       { (props.searchResults.length === 0 ? props.searchItems : props.searchResults).map(e => {
                return <ListItem id={e.id} button onClick={() => handleResultsClick(e)} disabled={e.disabled}>
                <ListItemIcon style={{minWidth:'50px'}} >
                  <Checkbox checked={isChecked(e)} onClick={(event)=>tourFunc(event)}/>
                </ListItemIcon>
                <ListItemIcon>
