...
import { handleEigenCreation } from 'store/sideBar/AnimationSlice/AllAnimationSlice';
...
export default function Eigen(){
  ...
  const eigenData = roots.filter((treeData:any) => {
    return treeData.pid === 'EIGEN';
  });

  const onHandleLabelAdd=(id:string,event:any)=> {

    if(id === AnimationType.EIGEN) {
  ...
  let dialogProps:DialogueProps={
    dialogueRun: true,
    tourName: TOUR_MENU_NAMES.EIGEN_VECTOR
  }
  ...
  const getBody = () => {
    ...
    return (
      ...
      <Button className={classes.button} onClick={(event)=>onHandleLabelAdd('EIGEN',event)}>
      <div className={classes.divIcon}>
      <LinearIcon fontSize="small" />
        </div>
        <div style={{marginLeft:'10px'}}>Add Eigen Vector</div> 
      </Button>
      </ButtonGroup>
  ...
  return (
          <SideBarContainer
            headerContent={ <Title text={"Eigen Vector" } group="animations"/>
             }
