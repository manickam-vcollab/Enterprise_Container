...
import { selectCheckedLeafNodes, selectEditableNodeId, selectSelectionPointerId, setEditableNodeId, setSelectionPointerId,handleLabel2DCreation, handle3dChartHeadCreation, setRouterHistory, handleChartHeadCreation ,setIsTitleEditable,setNewTitle,
  selectSearchHints,
  removeSearchHint, 
  selectPrevSearches,
  updatePrevSearches
...
export default function Label3DChart(){
  ...
  const checkedNodeIds  = useAppSelector(state => selectCheckedNodeForLabelType(state,LabelChartType.LINECHART));

  const data2D = roots.filter((treeData:any) => {
    return treeData.pid === LabelType.LABELCHART;
    //changed id to pid
  });
...
//      dispatch(setSelectionPointerId(''));
 ...
 // new Labelgui
  ...
  const getHeaderContent = () => {
    ...
    return (isSearchMode ?
      ...
      />
      : <Title text="2D Plots" group="Labels"/>)
  }
  ...
  const dataChart2D = roots.filter((treeData:any) => {
    return treeData.pid === LabelChartType.LINECHART;
  });
 ...
 useEffect(()=>{
  ...
  const getBody = () => {
    ...
    return (
      ...
      <div ref = {headerRef} >
        ...
        <Button className={classes.button}  onClick={(event)=>onHandleLabelAdd('LINECHART',event)}>
        <div className={classes.divIcon}>
        <XYPlotsIcon fontSize="small"/>
          </div>
          <div style={{marginLeft:'10px'}}>Add 2D Charts</div> 
        </Button>
        </ButtonGroup>
        </div>
  ...
  const getFooter = () => {
    ...
    // function to check weather parent label LineChart is checked or not
    // const isParentLabelChaecked = () => (checkedNodeIds.filter(i => (treeDataRedux[i].pid === LabelChartType.LINECHART)).length > 0)
