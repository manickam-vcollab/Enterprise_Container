...
export default function LabelList(){
  ...
  const checkedNodeIds  = useAppSelector(state => selectCheckedNodeForLabelType(state,Label3DType.PROBE));
  ...
  const ProbeData = roots.filter((treeData:any) => {
  ...
  useEffect(() => {
    ...
    return () =>{
      ...
      console.log('Active Layer manks',activeLayer);
    }
  },[])
 ...
 // new Labelgui
  ...
  const getHeaderContent = () => {
    ...
    return (isSearchMode ?
      ...
      />
      : <Title text="Point Label" group="Labels"/>)
  }
 ...
 useEffect(()=>{
  ...
  const onHandleLabelAdd=(id:string,event:any)=> {
    ...
    if(id === Label3DType.PROBE) {
      ...
      dispatch(handleProbeHeadCreation({data:event,undoable: true}))
      setInteractionMode(InteractionMode.DEFAULT, viewerId);
      dispatch(setLabelInsertionState(false));
    }
    ...
    return (
      <div style={{marginTop:'10px'}} ref = {containerRef}>
        ...
        <div style={{display:"flex",justifyContent:"center"}}>
          ...
          <Button className={classes.button}  onClick={(event)=>onHandleLabelAdd('PROBE',event)}>
          <div className={classes.divIcon}>
            <PointLabelIcon fontSize="small"/>
          </div>
          <div style={{marginLeft:'10px'}}>Add Point Label</div> 
          </Button>
          </ButtonGroup>
        </div>
